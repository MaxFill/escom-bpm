...
import com.maxfill.model.users.UserModel;
...
public class UserBean extends BaseExplBeanGroups<User, UserGroups>{            
    ...
    public void changePassword() throws NoSuchAlgorithmException{
        String newPwl = getModel().getNewPassword().trim();
        String repPwl = getModel().getRepeatePassword().trim();
        if (!Objects.equals(newPwl, repPwl)){
            EscomBeanUtils.ErrorMsgAdd("Error", "PasswordsNotMatch", "");
            return;
        }
        String oldPwlMD5 = EscomUtils.encryptPassword(getModel().getOldPassword().trim());
        ...
        getModel().setNewPassword(null);
        getModel().setOldPassword(null);
        getModel().setRepeatePassword(null);
    ...
    public void doGetCountUsesItem(User user,  Map<String, Integer> rezult){
        rezult.put("Staffs", staffFacade.findStaffByUser(user).size());
    }    
    ...
    protected void checkAllowedDeleteItem(User user, Set<String> errors){
        ...
        if (!staffFacade.findStaffByUser(user).isEmpty()){
